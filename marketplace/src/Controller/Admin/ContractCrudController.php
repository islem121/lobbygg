...
class ContractCrudController extends AbstractController
{
    ...
    public function index(Request $request, ContractRepository $contractRepository, PaginatorService $paginatorService): Response
    {
        $page = $request->query->getInt('page', 1);
        $pagination = $paginatorService->paginate($contractRepository->findAllQuery(), $page, 10);
    ...
    #[Route('/search', name: 'app_admin_contract_search', methods: ['GET'])]
    public function search(Request $request, ContractRepository $contractRepository): Response
    {
        $query = $request->query->get('q', '');
        $sort = $request->query->get('sort', 'id');
        $direction = $request->query->get('direction', 'DESC');
        
        $contracts = $contractRepository->searchContracts($query, $sort, $direction);

        return $this->render('admin/contract/_list.html.twig', [
            'contracts' => $contracts,
        ]);
    }

    #[Route('/stats/data', name: 'app_admin_contract_stats_data', methods: ['GET'])]
    public function statsData(ContractRepository $contractRepository): Response
    {
        $stats = $contractRepository->getCountByCompany();
