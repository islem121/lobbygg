...
class UserController extends AbstractController
{
    ...
    public function index(Request $request, UserRepository $userRepository, PaginatorService $paginatorService): Response
    {
        $page = $request->query->getInt('page', 1);
        $pagination = $paginatorService->paginate($userRepository->findAllNonAdminsQuery(), $page, 10);
    ...
    #[Route('/search', name: 'app_admin_user_search', methods: ['GET'])]
    public function search(Request $request, UserRepository $userRepository): Response
    {
        $query = $request->query->get('q', '');
        $users = $userRepository->searchNonAdmins($query);

        return $this->render('admin/user/_list.html.twig', [
            'users' => $users,
        ]);
    }
