...
class CommunityController extends AbstractController
{
    ...
    public function readNotification(Notification $notification, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($notification->getUser() === $user) {
            ...
            // Redirections selon le type
            if ($notification->getPost()) {
                return $this->redirectToRoute('front_blog', ['q' => '@'.$notification->getPost()->getUser()->getUsername()]);
            }
            if (in_array($notification->getType(), ['doc_accepted', 'doc_refused'])) {
                if (in_array('ROLE_CLIENT', $user->getRoles())) {
                    return $this->redirectToRoute('front_client_requests');
                }
                return $this->redirectToRoute('front_sponsor_requests');
            }
            if ($notification->getType() === 'doc_received') {
                return $this->redirectToRoute('front_sponsor_requests');
            }
            if ($notification->getType() === 'contract_created') {
